...
class project:
    def __init__(self, p_id, res_dir, version, used_name, apk_path):
        ...
        # zyx
        self.directAct = os.path.join(self.res_dir, "1directAct.txt")
        if not os.path.exists(self.directAct):
            with open(self.directAct, "w") as f:
                f.writelines("")
        self.indirectAct = os.path.join(self.res_dir, "1indirectAct.txt")
        if not os.path.exists(self.indirectAct):
            with open(self.indirectAct, "w") as f:
                f.writelines("")
        self.clickAct = os.path.join(self.res_dir, "1clickAct.txt")
        if not os.path.exists(self.clickAct):
            with open(self.clickAct, "w") as f:
                f.writelines("")
